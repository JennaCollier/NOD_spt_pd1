...
```{r get gene signature of exhaustion}
...
```{r add Tcon data}
tcon_merge <- merge(luoma_tcon, boland_tcon)
prog_exh <- FindMarkers(pan4, ident.1 = "Tcon_prog", group.by="seurat_clusters", only.pos = T)%>%filter(p_val_adj<0.05)%>%arrange(desc(avg_log2FC))
prog_exh <- convertMouseGeneList(rownames(prog_exh))
tcon_merge <- AddModuleScore(tcon_merge, features = list(prog_exh), name = "tcon_prog", assay = "RNA")

p_miller_texh <- tcon_merge@meta.data %>% 
                  group_by(pathology, paper, sample) %>% 
                  summarize(avg = mean(tcon_prog1)) %>%
                  ggplot() + 
                  geom_violin(aes(x = pathology, 
                                  fill = pathology, 
                                  y = avg, 
                  ...
                  labs(y = "Progenitor Tcon gene signature from NOD") +
                  add_pvalue(data.frame(tcon_merge@meta.data %>% 
                                            group_by(pathology, paper, sample) %>% 
                                            summarize(avg = mean(tcon_prog1)) %>% 
                                            ungroup() %>%
...
eff_signature <- convertMouseGeneList(eff_markers %>% filter(avg_log2FC > 0) %>% rownames()) #convert the positive logFC genes to human
...
```{r cd4 Tfh signature violins}
tfh_markers <- FindMarkers(pan4, 
                           ident.1 = "Tcon_Tfh", 
                           group.by = "seurat_clusters") %>% 
    filter(p_val_adj<0.05) %>%
    arrange(desc(avg_log2FC))
tfh_signature <- convertMouseGeneList(tfh_markers %>% filter(avg_log2FC > 0) %>% rownames()) #convert the positive logFC genes to human
luoma_tcon <- AddModuleScore(luoma_tcon, features = list(tfh_signature), name = "tfh_signature")
boland_tcon <- AddModuleScore(boland_tcon, features = list(tfh_signature), name = "tfh_signature")

#temporary fix to adjust naming scheme
luoma_tcon$treatment <- factor(recode_factor(luoma_tcon$treatment, "CT" = "healthy", "NC" = "ICI", "C" = "ICI+colitis"), 
                              levels = c("healthy","ICI","ICI+colitis"))
boland_tcon$treatment <- factor(recode_factor(boland_tcon$treatment, "HC" = "healthy", "UC" = "colitis"), levels = c("healthy","colitis"))

cd4_merge <- merge(luoma_tcon, boland_tcon)
cd4_merge$treatment <- factor(cd4_merge$treatment, 
                              levels = c("healthy","colitis","ICI","ICI+colitis"))

stat.test <- data.frame(cd4_merge@meta.data %>%
                            group_by(treatment,sample,paper) %>%
                            summarize(tfh_signature1 = mean(tfh_signature1)) %>%
                            group_by(paper) %>%
                            t_test(tfh_signature1 ~ treatment) %>% 
                            add_xy_position())

p_luoma <- cd4_merge@meta.data %>%
    group_by(treatment, sample, paper) %>%
    summarize(tfh_signature1 = mean(tfh_signature1)) %>%
    ggplot(aes(x = treatment,
               y = tfh_signature1)) +
...
grid.arrange(p_boland, p_luoma, nrow = 1, widths = c(2,3))
```
...
texh <- FindMarkers(pan8, 
                       ident.1 = "CD8_texh",
                      group.by = "seurat_clusters") %>% 
            filter(p_val_adj < 0.05 & avg_log2FC > 0) %>%
            arrange(desc(avg_log2FC)) %>%
            top_n(150, wt = avg_log2FC)
ifn <- FindMarkers(pan8, 
                       ident.1 = "Interferon_sensing",
                      group.by = "seurat_clusters") %>% 
            filter(p_val_adj < 0.05 & avg_log2FC > 0) %>%
            arrange(desc(avg_log2FC)) %>%
            top_n(150, wt = avg_log2FC)
mem <- FindMarkers(pan8, 
                       ident.1 = "CD8_mem",
                      group.by = "seurat_clusters") %>% 
            filter(p_val_adj < 0.05 & avg_log2FC > 0) %>%
            arrange(desc(avg_log2FC)) %>%
            top_n(150, wt = avg_log2FC)
effmem <- FindMarkers(pan8, 
                       ident.1 = "CD8_effmem",
                      group.by = "seurat_clusters") %>% 
            filter(p_val_adj < 0.05 & avg_log2FC > 0) %>%
            arrange(desc(avg_log2FC)) %>%
            top_n(150, wt = avg_log2FC)
eff <- FindMarkers(pan8, 
                       ident.1 = "CD8_eff",
                      group.by = "seurat_clusters") %>% 
            filter(p_val_adj < 0.05 & avg_log2FC > 0) %>%
            arrange(desc(avg_log2FC)) %>%
...
cm <- FindMarkers(pan8, 
            ...
            arrange(desc(avg_log2FC)) %>%
            top_n(150, wt = avg_log2FC)
slec <- FindMarkers(pan8, 
                       ident.1 = "CD8_slec",
                      group.by = "seurat_clusters") %>% 
            filter(p_val_adj < 0.05 & avg_log2FC > 0) %>%
            arrange(desc(avg_log2FC)) %>%
            top_n(150, wt = avg_log2FC)
prolif <- FindMarkers(pan8, 
...
texh_signature <- convertMouseGeneList(rownames(texh)) #convert the positive logFC genes to human
cm_signature <- convertMouseGeneList( rownames(cm)) #convert the positive logFC genes to human
ifn_signature <- convertMouseGeneList(rownames(ifn)) #convert the positive logFC genes to human
mem_signature <- convertMouseGeneList(rownames(mem)) #convert the positive logFC genes to human
prolif_signature <- convertMouseGeneList(rownames(prolif)) #convert the positive logFC genes to human
eff_signature <- convertMouseGeneList(rownames(eff)) #convert the positive logFC genes to human
slec_signature <- convertMouseGeneList(rownames(slec)) #convert the positive logFC genes to human

seurat_merge <- merge(boland_cd8, list(corri, luoma_cd8))
seurat_merge$treatment <- factor(recode_factor(seurat_merge$treatment, "CT" = "healthy", "NC" = "ICI", "C" = "ICI+colitis", "HC" = "healthy", "UC" = "colitis"), 
                              levels = c("healthy","ICI","ICI+colitis", "colitis"))

seurat_merge <- AddModuleScore(seurat_merge, features = list(cm_signature), name = "cm_signature", assay = "RNA")
seurat_merge <- AddModuleScore(seurat_merge, features = list(texh_signature), name = "texh_signature", assay = "RNA")
seurat_merge <- AddModuleScore(seurat_merge, features = list(ifn_signature), name = "ifn_signature", assay = "RNA")
seurat_merge <- AddModuleScore(seurat_merge, features = list(prolif_signature), name = "prolif_signature", assay = "RNA")
seurat_merge <- AddModuleScore(seurat_merge, features = list(mem_signature), name = "mem_signature", assay = "RNA")
seurat_merge <- AddModuleScore(seurat_merge, features = list(eff_signature), name = "eff_signature", assay = "RNA")
seurat_merge <- AddModuleScore(seurat_merge, features = list(slec_signature), name = "slec_signature", assay = "RNA")

pan8 <- AddModuleScore(pan8, features = list(rownames(cm)), name = "cm_signature", assay = "RNA")
pan8 <- AddModuleScore(pan8, features = list(rownames(texh)), name = "texh_signature", assay = "RNA")
pan8 <- AddModuleScore(pan8, features = list(rownames(ifn)), name = "ifn_signature", assay = "RNA")
pan8 <- AddModuleScore(pan8, features = list(rownames(prolif)), name = "prolif_signature", assay = "RNA")
pan8 <- AddModuleScore(pan8, features = list(rownames(mem)), name = "mem_signature", assay = "RNA")
pan8 <- AddModuleScore(pan8, features = list(rownames(eff)), name = "eff_signature", assay = "RNA")
pan8 <- AddModuleScore(pan8, features = list(rownames(slec)), name = "slec_signature", assay = "RNA")
...
medians_2d <- seurat_merge@meta.data %>% 
    filter(!is.na(clonotype_count_bin)) %>%
    group_by(celltype, paper, treatment, sample, clonotype_count_bin) %>% 
    summarize(PD1_score_median = mean(texh_signature1), mem_score_median = mean(eff_signature1))
