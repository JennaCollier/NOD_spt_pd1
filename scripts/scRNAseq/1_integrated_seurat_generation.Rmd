...
treg1_genes <- seurat_markers %>% filter(cluster == "Treg_resting") %>% top_n(50, wt = avg_log2FC) %>% pull(gene) %>% unique()
treg2_genes <- seurat_markers %>% filter(cluster == "Treg_eff") %>% top_n(50, wt = avg_log2FC) %>% pull(gene) %>% unique()
acinar_genes <- seurat_markers %>% filter(cluster == "Acinar_contaminated") %>% top_n(50, wt = avg_log2FC) %>% pull(gene) %>% unique()
ifn_genes <- seurat_markers %>% filter(cluster == "Interferon_sensing") %>% filter(grepl("Is|If|Stat1", gene))
activation_genes <- seurat_markers %>% filter(cluster == "Recently_activated") %>% top_n(50, wt = avg_log2FC) %>% pull(gene) %>% unique()
proliferating_genes <- seurat_markers %>% filter(cluster == "Proliferating") %>% top_n(50, wt = avg_log2FC) %>% pull(gene) %>% unique()
```
...
seurat_integrated <- AddMetaData(seurat_integrated, new_metadata)
new_metadata <- seurat_integrated@meta.data %>% 
                  mutate(tetramer_specificity = ifelse(NRPv7_TotalSeqC > 1.5 & celltype == "CD8", #cutoff for specificity
                                                 "NRPv7", 
                                                 "unknown")) %>%
...
tetramer_tcrs <- list() #list containing all of the tcr data

for (condition in conditions) {
  tetramer_tcrs <- read.table(paste0("data/GSE151652", sample_name))$V1
}
