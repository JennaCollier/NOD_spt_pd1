...
for (i in 1:length(list.files(path = "data/nod/"))) {
  condition <- list.files(path = "data/nod/")[i]
  
  tcrdata <- read.csv(paste0("data/nod/", condition, "/filtered_contig_annotations.csv"), header = T)
  ...
  clonotype_data <- read.csv(paste0("data/nod/", condition, "/consensus_annotations.csv"), header = T)
...
seurat_integrated@meta.data %>%
  ...
  summarize(cd4_percent = 100*sum(Cd4 > 0)/n(), 
            cd8_percent = 100*sum(Cd8a > 0)/n(), 
            cd3_percent = 100*sum(Cd3e > 0)/n()) %>% 
  mutate(to_be_filtered = cd4_percent < 10 & cd8_percent < 10 | cd3_percent < 80) %>% 
  filter(to_be_filtered == TRUE)
...
write.table(seurat_markers %>% group_by(cluster) %>% top_n(50, wt = avg_log2FC), "output/all_clusters_top50markers.tsv", sep = "\t", quote = F, row.names = F) #save to file
...
```{r Save list of TCRs}
write.csv(seurat_integrated@meta.data %>% 
            dplyr::select(treatment, tissue, mouse_id, celltype, clonotype_id, TRA.v_gene, TRA.j_gene, TRA.c_gene, TRA.cdr3_nt, TRA.cdr3, TRB.v_gene, TRB.j_gene, TRB.c_gene, TRB.cdr3_nt, TRB.cdr3) %>% 
            filter(!is.na(clonotype_id)),
          file = "output/tableS7.csv")
```
