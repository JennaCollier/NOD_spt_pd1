...
Add the NRP-v7 tetramer staining to the Seurat meta data for ease of plotting. An NRP-v7+ T cell was identified if: 1) The cell was classified as a CD8 T cell; 2) The cell had an associated TCR; 3) the cell had a NRP-7 tetramer TotalSeqC staining value of over 0.5, and; 4) if, based on the previous criteria, there were more positive than negative NRP-v7 T cells in the tcrtype (determined by paired TRA.cdr3 and TRB.cd3 sequences only).
```{r }
new_metadata <- as.data.frame(t(seurat_integrated@assays$FB@data))
colnames(new_metadata) <- "NRPv7_TotalSeqC"
seurat_integrated <- AddMetaData(seurat_integrated, new_metadata)
new_metadata <- seurat_integrated@meta.data %>% 
                  mutate(tetramer_specificity = ifelse(NRPv7_TotalSeqC > 1 & celltype == "CD8", #cutoff for specificity
                                                 "NRPv7", 
                                                 "unknown")) %>%
...
tbl <- data.frame(seurat_integrated@meta.data %>%
        ...
        group_by(tetramer_specificity, tcrtype_id) %>% 
        tally() %>% 
        cast(tcrtype_id ~ tetramer_specificity, value = "n")) %>%
        arrange(desc(NRPv7))
tbl[is.na(tbl)] <- 0
