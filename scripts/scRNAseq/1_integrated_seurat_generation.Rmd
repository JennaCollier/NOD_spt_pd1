...
cd8$treatment <- factor(cd8$treatment, levels = c("IgG", "Spt", "PD1"))
cd4$treatment <- factor(cd4$treatment, levels = c("IgG", "Spt", "PD1"))
```
...
new_metadata <- data.frame(coinh_count = rowSums(as.data.frame(t(as.data.frame(pan8@assays$RNA@data[c("Pdcd1","Havcr2","Ctla4","Lag3","Tigit"),]))) > 0))
seurat_integrated <- AddMetaData(seurat_integrated, new_metadata)
```
...
new_metadata <- seurat_integrated@meta.data %>% 
                  mutate(cd4_or_cd8 = paste0(celltype), 
                         celltype = ifelse(seurat_clusters %in% c("Treg_Tigit_lo","Treg_Tigit_hi"), "Treg", paste0(celltype))) %>% 
...
shared_clonotypes <- seurat_integrated@meta.data %>% 
  ...
  tally() %>% 
  summarize(clonotype_tissue_groups = paste(tissue, collapse='&')) %>% 
...
new_metadata <- new_metadata %>%
                  mutate(tissue_shared = case_when(is.na(clonotype_tissue_groups) ~ NA_character_,
                                                   tissue == "blood" & clonotype_tissue_groups %in% c("pancreas&pLN&blood", "pancreas&blood") ~ "blood shared",
                                                   tissue == "pLN"  & clonotype_tissue_groups %in% c("pancreas&pLN&blood", "pancreas&pLN") ~ "pLN shared",
                                                   tissue == "pancreas"  & clonotype_tissue_groups %in% c("pancreas&pLN&blood", "pancreas&pLN", "pancreas&blood") ~ "pancreas shared",
                                                   tissue == "pancreas"  & clonotype_tissue_groups == "pancreas" ~ "pancreas non shared",
                                                   tissue != "pancreas"  & clonotype_tissue_groups %in% c("pLN&blood", "pLN", "blood") ~ "periphery non shared"))
seurat_integrated <- AddMetaData(seurat_integrated, new_metadata)
seurat_integrated$tissue_shared <- factor(seurat_integrated$tissue_shared, levels = c("pancreas shared", "pancreas non shared", "pLN shared", "blood shared", "periphery non shared"))
```

Finally, save the Seurat objects:
