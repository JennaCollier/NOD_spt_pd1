...
p_blood_shared <- seurat_integrated@meta.data %>% 
                    filter(tissue == "blood" & !is.na(clonotype_shared_tissues)) %>%
                    ...
                    guides(color = guide_legend(override.aes = list(alpha = 1, 
                                                                  size = 5, 
                                                                  shape = 19))) +
                  facet_grid(rows = vars(clonotype_shared_tissues), 
                             cols = vars(treatment))
p_blood_shared

#PIES OF CLUSTERS IN EACH GROUP
p_cd8_blood_pie <- seurat_integrated@meta.data %>% 
                    filter(tissue == "blood" & !is.na(clonotype_shared_tissues) & celltype == "CD8") %>%
                    group_by(treatment,clonotype_shared_tissues,
                             seurat_clusters) %>% 
                    tally() %>% 
                    group_by(treatment, clonotype_shared_tissues) %>% 
...
p_cd4_blood_pie <- seurat_integrated@meta.data %>% 
                    filter(tissue == "blood" & !is.na(clonotype_shared_tissues) & celltype %in% c("Tcon","Treg")) %>%
                    group_by(treatment,clonotype_shared_tissues,
                             seurat_clusters) %>% 
                    tally() %>% 
                    group_by(treatment, clonotype_shared_tissues) %>% 
...
#TOP 50 CLONOTYPES IN BLOOD
#CD8
cumulative_sums <- seurat_integrated@meta.data %>% 
                    filter(clonotype_tissue_groups %in% c("blood&pancreas","pancreas") & celltype == "CD8") %>%
                      group_by(treatment, clonotype_id, clonotype_tissue_groups) %>% 
                      tally() %>% 
                      group_by(treatment) %>% 
                      mutate(prcnt = n*100/sum(n)) %>% 
                      group_by(treatment, clonotype_tissue_groups) %>% 
                      arrange(treatment, clonotype_tissue_groups, desc(prcnt)) %>% 
...
p_cd8_bloodPM_top50_clonos <- cumulative_sums %>% 
                              group_by(treatment, clonotype_tissue_groups) %>% 
                              slice_head(n = 50) %>% 
                              ...
                              facet_grid(treatment ~ clonotype_tissue_groups, scales = "free_x", ncol = 2)
...
p_tcon_bloodPM_top50_clonos <- cumulative_sums %>% 
                              group_by(treatment, clonotype_tissue_groups) %>% 
                              slice_head(n = 50) %>% 
                              ggplot(aes(x = clonotype_id, 
                                         y = cs, 
                              ...
                              facet_wrap(treatment ~ clonotype_tissue_groups, scales = "free_x", ncol = 2)
...
p_treg_bloodPM_top50_clonos <- cumulative_sums %>% 
                              group_by(treatment, clonotype_tissue_groups) %>% 
                              slice_head(n = 50) %>% 
                              ggplot(aes(x = clonotype_id, 
                                         y = cs, 
...
grid.arrange(p_cd8_bloodPM_top50_clonos, p_tcon_bloodPM_top50_clonos, p_treg_bloodPM_top50_clonos, nrow = 1)

#boxplot of clone size
stat.test <- data.frame(seurat_integrated@meta.data %>%
                          filter(tissue == "pancreas" & !is.na(clonotype_shared_tissues)) %>%
                          group_by(treatment, clonotype_shared_tissues, clonotype_id) %>%
                          tally() %>%
                          group_by(treatment) %>%
                          wilcox_test(n ~ clonotype_shared_tissues) %>% 
                          add_xy_position() %>%
                          mutate(y.position = log10(y.position + 1)))

seurat_integrated@meta.data %>%
    filter(tissue == "pancreas" & !is.na(clonotype_shared_tissues)) %>%
    group_by(celltype, treatment, clonotype_shared_tissues, clonotype_id) %>%
    tally() %>%
    ggplot() + 
