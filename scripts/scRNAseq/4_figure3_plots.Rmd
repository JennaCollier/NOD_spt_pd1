...
p_cd4_panonly_cluster <- pan4@meta.data %>% 
                            ...
                            labs(x = 'UMAP 1', 
                                 ...
                                 title = "CD4+ T cells in the pancreas",
...
grid.arrange(p_cd4_panonly_cluster, p_cd4_panonly_treatments, widths = c(3, 6))
```
...
cumulative_sums[[cell]] <- pan4@meta.data %>% 
                            filter(!is.na(clonotype_id) & celltype == "Tcon") %>% 
...
p_top50_clonos[[cell]] <- cumulative_sums[[cell]] %>% 
                            ...
                            labs(x = "Top 50 clonotypes", 
                                 ...
                                 title = paste0(cell, " in pancreas")) +
                            facet_wrap(~ treatment, scales = "free_x")
}
grid.arrange(p_top50_clonos[["Tcon"]], p_top50_clonos[["Treg"]], nrow = 2)

#boxplot of clone size

stat.test <- data.frame(pan4@meta.data %>%
...
df$cluster <- factor(df$cluster, levels = levels(pan4$seurat_clusters))
